...
$result = $stmt->execute(array('fname' => $_POST['firstName'], 'lname' => $_POST['lastName'], 'username' => $_POST['email'], 'password' => md5($_POST['pass']), 'role' => 'buyer', 'emails' => 5, 'emailspermonth' => 500, 'timestamp' => date('M d, Y g:i A')));
	...
	while($row2 = $stmt2->fetch()){
		$stmt3 = $conn->prepare('INSERT INTO templates (user_id, name, type, picture, value) VALUES (:userid, :name, :type, :picture, :value)');
		$stmt3->execute(array('userid' => $_GET['id'], 'name' => $row2['name'], 'type' => $row2['type'], 'picture' => $row2['picture'], 'value' => $row2['value']));
	}
