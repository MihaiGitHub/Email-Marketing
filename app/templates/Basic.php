<table cellspacing="0" border="1" style="background-image: url(../app/images/bg.gif); background-color: #c5c5c5;" cellpadding="0" width="100%">	
	<tr>
		<td valign="top">
			<table cellspacing="0" border="0" align="center" style="background: #fff; border-right: 1px solid #ccc; border-left: 1px solid #ccc;" cellpadding="0" width="650">		
				<tr>
					<td>
						<!-- content -->
						<table cellspacing="0" border="0" height="470" cellpadding="0" width="650">
							<tr>
								<td valign="top" width="270">	
									<table cellspacing="0" cellpadding="0">
										<tr>
											<td style="padding: 0 0 0 20px;">
												<table cellspacing="0" cellpadding="0">
													<tr>
														<td>
															<br/>
									<table cellspacing="0" border="0" cellpadding="0">										
										<tr>
											<td class="content-copy" style="padding: 0 20px; color: #000; font-size: 14px; font-family: Georgia; line-height: 20px;">
								<div class='inline-edit'>
									<div class='display'><?php echo (array_key_exists(-9, $fields)) ? nl2br($fields[-9]) : 'Aenean pulvinar est at nibh commodo vel vehicula quam pharetra. Donec in ante lorem. Nam ullamcorper 
												luctus laoreet. Morbi quis augue vitae quam pharetra varius at at urna. Nullam 
												tempor pulvinar erat ut blandit. Nam et erat diam, vel egestas enim.'; ?>
									</div>
									<div class='form'>
										<div><div><textarea class='text'></textarea></div></div>
										<div>
											<input type='submit' class='save' value=' Save ' />
											<input type='submit' class='cancel' value=' Cancel ' />
										</div>
									</div>
								</div>
											</td>
										</tr>
									</table>
								</td>
							</tr>
						</table>
						<!--  / content -->
					</td>
				</tr>
			</table>
		</td>
	</tr>
</table>