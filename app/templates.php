...
if($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['type'] != 'all'){
	...
	$stmt = $conn->prepare('SELECT id, name, type, picture, value, path FROM templates WHERE user_id = :userid AND type = :type ORDER BY id DESC');
	$result = $stmt->execute(array('userid' => $_SESSION['id'], 'type' => $_POST['type']));

} else {

	$stmt = $conn->prepare('SELECT id, name, type, picture, value, path FROM templates WHERE user_id = :userid ORDER BY id DESC');
