...
<title>PayPal Mass Payment History - Sent Payments</title>
  ...
  <div id="content">
    <div class="WADAResultsContainer"> <a name="top" id="top"></a>
      <div id="WADAPageTitleArea">
        <div id="WADAPageTitle">PayPal Mass Payments</div>
        <div><a href="mass-payments-search.php">New Search</a> | <a href="mass-payments-results.php?show_all=1">Show All</a></div>
      </div>
      ...
      <div class="WADAResults">
        <div class="WADAResultsNavigation">
          <div class="WADAResultsCount">Records <?php echo ($startRow_WADApaypal_mass_payments + 1) ?> to <?php echo min($startRow_WADApaypal_mass_payments + $maxRows_WADApaypal_mass_payments, $totalRows_WADApaypal_mass_payments) ?> of <?php echo $totalRows_WADApaypal_mass_payments ?> </div>
          <div class="WADAResultsNavTop">
            <table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
              <tr valign="middle">
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments > 0) { // Show if not first page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, 0, $queryString_WADApaypal_mass_payments); ?>" title="First"><img border="0" name="First" id="First" alt="First" src="../WA_DataAssist/images/Pacifica/Refined_first.gif" /></a>
                  <?php } // Show if not first page ?></td>
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments > 0) { // Show if not first page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, max(0, $pageNum_WADApaypal_mass_payments - 1), $queryString_WADApaypal_mass_payments); ?>" title="Previous"><img border="0" name="Previous" id="Previous" alt="Previous" src="../WA_DataAssist/images/Pacifica/Refined_previous.gif" /></a>
                  <?php } // Show if not first page ?></td>
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments < $totalPages_WADApaypal_mass_payments) { // Show if not last page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, min($totalPages_WADApaypal_mass_payments, $pageNum_WADApaypal_mass_payments + 1), $queryString_WADApaypal_mass_payments); ?>" title="Next"><img border="0" name="Next" id="Next" alt="Next" src="../WA_DataAssist/images/Pacifica/Refined_next.gif" /></a>
                  <?php } // Show if not last page ?></td>
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments < $totalPages_WADApaypal_mass_payments) { // Show if not last page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, $totalPages_WADApaypal_mass_payments, $queryString_WADApaypal_mass_payments); ?>" title="Last"><img border="0" name="Last" id="Last" alt="Last" src="../WA_DataAssist/images/Pacifica/Refined_last.gif" /></a>
                  <?php } // Show if not last page ?></td>
              </tr>
            </table>
          </div>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="WADAResultsTable">
          <tr>
            ...
            <th width="11%" class="WADAResultsTableHeader">Total</th>
            <th width="11%" class="WADAResultsTableHeader">Fee</th>
            <th width="18%" class="WADAResultsTableHeader">Receiver Email Address</th>
          ...
          <tr 
		  <?php 
		  if($row_WADApaypal_mass_payments['test_ipn'] == 1) 
		  	echo 'style="background-color:#FF9"';
		  elseif($row_WADApaypal_mass_payments['ipn_status'] == 'Invalid')
		  	echo 'style="background-color:#F33; color:#FF9;"';
			?> 
            class="<?php echo $WARRT_AltClass1->getClass(true); ?>">
            <td class="WADAResultsTableCell"><?php 
			echo date('m.d.Y g:i:s A', strtotime($row_WADApaypal_mass_payments['creation_timestamp']));
			?></td>
            <td class="WADAResultsTableCell"><?php echo($row_WADApaypal_mass_payments['masspay_txn_id']); ?></td>
            <td class="WADAResultsTableCell">$<?php echo(number_format($row_WADApaypal_mass_payments['mc_gross'],2)); ?></td>
            <td class="WADAResultsTableCell">$<?php echo(number_format($row_WADApaypal_mass_payments['mc_fee'],2)); ?></td>
            <td class="WADAResultsTableCell"><?php echo($row_WADApaypal_mass_payments['receiver_email']); ?></td>
            <td class="WADAResultsTableCell"><?php echo($row_WADApaypal_mass_payments['status']); ?></td>
            <td class="WADAResultsEditButtons" nowrap="nowrap"><table class="WADAEditButton_Table">
              <tr>
                <td><a href="mass-payments-detail.php?id=<?php echo(rawurlencode($row_WADApaypal_mass_payments['id'])); ?>" title="View"><img border="0" name="View<?php echo(rawurlencode($row_WADApaypal_mass_payments['id'])); ?>" id="View<?php echo(rawurlencode($row_WADApaypal_mass_payments['id'])); ?>" alt="View" src="../WA_DataAssist/images/Pacifica/Refined_zoom.gif" /></a></td>
                <td><a href="mass-payments-delete.php?id=<?php echo(rawurlencode($row_WADApaypal_mass_payments['id'])); ?>" title="Delete"><img border="0" name="Delete<?php echo(rawurlencode($row_WADApaypal_mass_payments['id'])); ?>" id="Delete<?php echo(rawurlencode($row_WADApaypal_mass_payments['id'])); ?>" alt="Delete" src="../WA_DataAssist/images/Pacifica/Refined_trash.gif" /></a></td>
                </tr>
            </table></td>
          </tr>
          <?php } while ($row_WADApaypal_mass_payments = mysql_fetch_assoc($WADApaypal_mass_payments)); ?>
        </table>
        <div class="WADAResultsNavigation">
          <div class="WADAResultsCount">Records <?php echo ($startRow_WADApaypal_mass_payments + 1) ?> to <?php echo min($startRow_WADApaypal_mass_payments + $maxRows_WADApaypal_mass_payments, $totalRows_WADApaypal_mass_payments) ?> of <?php echo $totalRows_WADApaypal_mass_payments ?> </div>
          <div class="WADAResultsNavBottom">
            <table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
              <tr valign="middle">
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments > 0) { // Show if not first page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, 0, $queryString_WADApaypal_mass_payments); ?>" title="First"><img border="0" name="First1" id="First1" alt="First" src="../WA_DataAssist/images/Pacifica/Refined_first.gif" /></a>
                  <?php } // Show if not first page ?></td>
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments > 0) { // Show if not first page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, max(0, $pageNum_WADApaypal_mass_payments - 1), $queryString_WADApaypal_mass_payments); ?>" title="Previous"><img border="0" name="Previous1" id="Previous1" alt="Previous" src="../WA_DataAssist/images/Pacifica/Refined_previous.gif" /></a>
                  <?php } // Show if not first page ?></td>
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments < $totalPages_WADApaypal_mass_payments) { // Show if not last page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, min($totalPages_WADApaypal_mass_payments, $pageNum_WADApaypal_mass_payments + 1), $queryString_WADApaypal_mass_payments); ?>" title="Next"><img border="0" name="Next1" id="Next1" alt="Next" src="../WA_DataAssist/images/Pacifica/Refined_next.gif" /></a>
                  <?php } // Show if not last page ?></td>
                <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADApaypal_mass_payments < $totalPages_WADApaypal_mass_payments) { // Show if not last page ?>
                  <a href="<?php printf("%s?pageNum_WADApaypal_mass_payments=%d%s", $currentPage, $totalPages_WADApaypal_mass_payments, $queryString_WADApaypal_mass_payments); ?>" title="Last"><img border="0" name="Last1" id="Last1" alt="Last" src="../WA_DataAssist/images/Pacifica/Refined_last.gif" /></a>
                  <?php } // Show if not last page ?></td>
              </tr>
            </table>
          </div>
